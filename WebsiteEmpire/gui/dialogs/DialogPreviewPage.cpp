...
void DialogPreviewPage::_renderPage(const PreviewEntry &entry)
{
    ...
    _inlineSvgs(html, domain, entry.lang);
    _inlineRasterImages(html, domain, entry.lang);

    // html is a complete HTML document generated by AbstractPageType::addCode.
    const QString &fullHtml = html;
...
void DialogPreviewPage::_inlineSvgs(QString &html, const QString &domain, const QString &lang)
{
                           ...
                           + QString::number(reinterpret_cast<quintptr>(this));
    {
        ...
        if (db.open()) {
            for (const QString &fn : std::as_const(filenames)) {
                QSqlQuery q(db);
                // Priority: translated (domain=lang) > source domain > domain="" fallback.
                q.prepare(QStringLiteral(
                    "SELECT b.blob FROM images b"
                    " JOIN image_names n ON n.image_id = b.id"
                    " WHERE n.filename = :fn"
                    "   AND (n.domain = :lang OR n.domain = :domain OR n.domain = '')"
                    " ORDER BY CASE WHEN n.domain = :lang   THEN 0"
    ...
    while (it.hasNext()) {
        ...
        // Embed as SVG data URI — no rasterisation, crisp at any display size.
        const QString dataUri = QStringLiteral("data:image/svg+xml;base64,")
                              + QString::fromLatin1(blobIt.value().toBase64());
